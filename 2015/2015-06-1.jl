input = raw"turn on 489,959 through 759,964
...
function solve(input)
	...
	regex = r"([a-z\s]+)([\d]+),([\d]+) through ([\d]+),([\d]+)"
	for cmd in split(input, "\n")
		m = match(regex, cmd)
		if m[1] == "toggle "
			for x in to_index(m[2]):to_index(m[4]), y in to_index(m[3]):to_index(m[5])
				lights[x,y] = !lights[x,y]
			end
		elseif m[1] == "turn on "
			(lights[to_index(m[2]):to_index(m[4]), to_index(m[3]):to_index(m[5])]).=true
		elseif m[1] == "turn off "
			(lights[to_index(m[2]):to_index(m[4]), to_index(m[3]):to_index(m[5])]).=false
		else
			println("unknown cmd: $m([1])")
		end
	end
	return count(i -> i, lights)
end

to_index(str) = parse(Int16, str)+1
