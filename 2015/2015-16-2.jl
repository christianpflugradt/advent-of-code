input = raw"Sue 1: goldfish: 9, cars: 0, samoyeds: 9
...
function solve(input)
	regex = r"Sue ([\d]+): ([a-z]+): ([-\d]+), ([a-z]+): ([-\d]+), ([a-z]+): ([-\d]+)"
	sue = Dict()
	sue["children"] = 3
	sue["cats"] = 7
	sue["samoyeds"] = 2
	...
	for line in split(input, "\n")
		m = match(regex, line)
		matches_all = true
		for (k,v) in [(m[2], m[3]), (m[4], m[5]), (m[6], m[7])]
			if in(k, ["cats", "trees"])
				if sue[k] >= parse(Int, v)
					matches_all = false
				end
			elseif in(k, ["pomeranians", "goldfish"])
				if sue[k] <= parse(Int, v)
					matches_all = false
				end
			else
