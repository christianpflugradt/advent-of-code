...
findMaxRegVal :: Int -> M.Map String Int -> [Instruction] -> Int
findMaxRegVal mx registers [] = mx
findMaxRegVal mx registers instructions = findMaxRegVal mxNew updatedRegisters (tail instructions)
    where
        mxNew = max mx (if M.null registers then 0 else maximum (M.elems registers))
...
solve :: String -> Int
solve = findMaxRegVal 0 M.empty . map parse . splitLines
