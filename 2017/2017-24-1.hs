...
data Bridge = Bridge { strength :: Int, pins :: Int, remaining :: [Component] }
...
initialBridge comps = Bridge 0 0 comps
...
findAllBridges bridge =
    ...
    then [bridge]
        ...
        let b = Bridge { strength = strength bridge + uncurry (+) c, pins = nextPins, remaining = delete c (remaining bridge) }
        ]
    where parts = matches (pins bridge) (remaining bridge)

solve :: String -> Int
solve = maximum . map strength . findAllBridges . initialBridge . toComponents
