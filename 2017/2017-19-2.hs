...
travelAndCollect :: Diagram -> Coordinate -> Int
travelAndCollect diagram (xpos, ypos) = step (xpos, ypos, Downward) 0
    where
        step (x, y, dir) steps
            | isNothing cell = steps
            | Just (Line _) <- cell = step (nxt dir) newSteps
            | Just (Letter c) <- cell = step (nxt dir) newSteps
            | Just Corner <- cell, vertical = step (nxt $ Along Horizontal) newSteps
            | Just Corner <- cell = step (nxt $ Along Vertical) newSteps
            where
                newSteps = inc steps
