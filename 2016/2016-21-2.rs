use regex::Regex;
use crate::s_2016_21_2::Direction::{Left, Right};
use crate::s_2016_21_2::Instruction::{Move, Reverse, RotateBy, RotateSteps, SwapLetters, SwapPositions};

const ORIGINAL: &str = "fbgdceah";
...
impl Scrambler {
    ...
    fn rotate_steps(password: &str, steps: usize, direction: Direction) -> String {
        ...
        let split = if direction == Right { n } else { password.len() - n };
        let (left, right) = password.split_at(split);
        format!("{}{}", right, left)
    }

    fn rotate_by_original(password: &str, letter: char) -> String {
        match password.find(letter) {
            Some(index) => {
                Self::rotate_steps(password, if index >= 4 { index + 2 } else { index + 1 }, Left)
            },
    ...
    fn rotate_by(password: &str, letter: char) -> String {
        for step in 1..10 {
            let prev = Self::rotate_steps(password, step, Right);
            if Self::rotate_by_original(&prev, letter) == password {
                return prev.to_string()
            }
        }
        panic!("cannot reverse rotate_by for {} in {}", letter, password)
    }
